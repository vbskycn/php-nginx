...
# 验证所有配置文件
echo "验证配置文件..."
nginx -t && echo "Nginx配置验证成功" || (echo "Nginx配置验证失败" && exit 1)
php-fpm84 -t && echo "PHP-FPM配置验证成功" || (echo "PHP-FPM配置验证失败" && exit 1)

# 创建配置完成标志文件
touch /tmp/configure_complete
echo "配置完成标志文件已创建"

# 显示当前配置摘要
echo "=== 配置摘要 ==="
echo "PHP内存限制: $PHP_MEMORY_LIMIT"
echo "PHP错误显示: $PHP_DISPLAY_ERRORS"
echo "PHP最大执行时间: ${PHP_MAX_EXECUTION_TIME}秒"
echo "OPcache启用: $OPCACHE_ENABLE"
echo "OPcache内存: ${OPCACHE_MEMORY_CONSUMPTION}MB"
echo "PHP-FPM进程模式: $PHP_FPM_PM_MODE"
